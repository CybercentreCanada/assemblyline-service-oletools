...
if (sys.version_info > (3,)):
    ...
    from oletools.olevba3 import decompress_stream
    xrange = range
    def ord(x):
        return x
    def decode(x):
...
def getTheIdentifiers(vbaProjectData):
    ...
    try:
        ...
        raise Exception('getTheIdentifiers {}.'.format(e))
    return identifiers
...
def dumpLine(moduleData, lineStart, lineLength, endian, vbaVer, is64bit, identifiers, objectTable, indirectTable, declarationTable, verbose, line):
    ...
    line_res.append('Line #{:d}:'.format(line))
    if (lineLength <= 0):
        str_results = "\n".join(line_res)
        return str_results
    if (verbose):
    ...
    while (offset < endOfLine):
        ...
        if (not translatedOpcode in opcodes):
            line_res.append('Unrecognized opcode 0x{:04X} at offset 0x{:08X}.'.format(opcode, offset))
            return line_res
        instruction = opcodes[translatedOpcode]
        mnemonic = instruction['mnem']
        line_part = '\t'
        if (verbose):
            line_part += '{:04X} '.format(opcode)
        line_part += '{} '.format(mnemonic)
        if (mnemonic in ['Coerce', 'CoerceVar', 'DefType']):
            if (opType < len(varTypesLong)):
                line_part += '({}) '.format(varTypesLong[opType])
            elif (opType == 17):
                line_part += '(Byte) '
            else:
                line_part += '({:d}) '.format(opType)
            ...
            if (len(dimType)):
                line_part += '({}) '.format(' '.join(dimType))
        elif (mnemonic == 'LitVarSpecial'):
            line_part += '({})'.format(specials[opType])
            ...
            line_part += ' ({})'.format(options[opType])
        ...
        for arg in instruction['args']:
            if (arg == 'name'):
                ...
                if   ((arg == 'rec_') and (len(indirectTable) >= dword + 20)):
                    ...
                    line_part += '{}'.format(theFunc)
                else:
                    line_part += '{}{:08X} '.format(arg, dword)
                if (is64bit and (arg == 'context_')):
                    offset, dword = getVar(moduleData, offset, endian, True)
                    line_part += '{:08X} '.format(dword)
        if (instruction['varg']):
            ...
            line_part += '{}'.format(theVarArg)
            offset += wLength
            if (wLength & 1):
                offset += 1
        line_res.append(line_part)
...
def pcodeDump(moduleData, vbaProjectData, dirData, identifiers, is64bit, verbose, disasmOnly):
    ...
    # Office 2010 is 0x0097; Office 2013 is 0x00A3;
        ...
        for line in range(numLines):
            ...
            line_res = dumpLine(moduleData, pcodeStart + lineOffset, lineLength, endian, vbaVer, is64bit, identifiers,
                                objectTable, indirectTable, declarationTable, verbose, line)
            pcode_res.append(line_res)
    except Exception as e:
        raise Exception("Pcode compile error: {}" .format(e))
...
def process_doc(vbaParser, verbose=False, disasmOnly=True):
    results = []
    str_res = processProject(vbaParser, results, verbose, disasmOnly)
    return str_res
